...
begin
  ...
  zCon.Database:=Utils.pCaminhoBdLogs;
...
function TdmCon.ConectaBD(Connect: boolean): Boolean;
begin
  try
    if (Connect) then
      zCon.Connect
...
procedure TdmCon.FechaConexoes;
begin
  zCon.Disconnect;
end;

procedure TdmCon.GetStatus(out Resp : TJSONData);
...
procedure TdmCon.PesquisaLogs(Req: TJSONObject; out Resp: TJSONData);
var
  vMsg,vFun : string;
  vOk : Boolean;
  TagJson,Item : string;
...
procedure TdmCon.GravaLogBD(Req: TJSONObject; out Resp: TJSONData);
var
  vMsg,vFun : string;
  vOk : Boolean;
  TagJson : string;
